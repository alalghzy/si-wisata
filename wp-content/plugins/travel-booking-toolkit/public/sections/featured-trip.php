...
if ( $title || $content || $obj->travel_booking_toolkit_is_wpte_activated() ) { ?>
	<section id="featured-trip-section" class="featured-trip toolkit-trip">
		<div class="container">
			<?php
			if ( $title || $content ) {
				?>
					<header class="section-header">
						<?php
						if ( $title ) {
							echo '<h2 class="section-title">' . esc_html( travel_booking_toolkit_get_featured_title() ) . '</h2>';
						}
						if ( $content ) {
							echo '<div class="section-content">' . wp_kses_post( travel_booking_toolkit_get_featured_content() ) . '</div>';
						}
						?>
					</header>
					<?php
