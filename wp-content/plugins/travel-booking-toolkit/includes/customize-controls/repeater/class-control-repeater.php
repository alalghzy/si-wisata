<?php
/**
 * Travel Booking Toolkit Customizer Repeater Control.
 * 
...
if( ! class_exists( 'Travel_Booking_Toolkit_Control_Repeater' ) ) {
    ...
    class Travel_Booking_Toolkit_Control_Repeater extends WP_Customize_Control {
    	 ...
    	 * @var string
    	 */
    	public $type = 'travel-booking-toolkit-repeater';
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		$this->row_label = array(
    			'type' => 'text',
    			'value' => esc_attr__( 'row', 'travel-booking-toolkit' ),
    			'field' => false,
    		);
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'travel-booking-toolkit' ), $this->row_label['value'] );
    		}
    	...
    	public function enqueue() {    
           ...
           wp_enqueue_script( 'travel-booking-toolkit-repeater', TBT_FILE_URL . '/includes/customize-controls/repeater' . $build . '/repeater' . $suffix . '.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
            wp_enqueue_style( 'travel-booking-toolkit-repeater', TBT_FILE_URL . '/includes/customize-controls/repeater' . $build . '/repeater' . $suffix . '.css', null );

             wp_localize_script( 'travel-booking-toolkit-repeater', 'travel_booking_toolkit_customize',
                array(
                    'nonce' => wp_create_nonce( 'travel_booking_toolkit_customize_nonce' )
                )
            );
    	}
    	...
    	protected function render_content() {
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'travel-booking-toolkit' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row="{{{ index }}}">
    				...
    				<div class="repeater-row-content">
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'travel-booking-toolkit' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="travel-booking-toolkit-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'travel-booking-toolkit' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php esc_attr_e( 'No Image Selected', 'travel-booking-toolkit' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'travel-booking-toolkit' ); ?></button>
    									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'travel-booking-toolkit' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'travel-booking-toolkit' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_attr_e( 'Change Image', 'travel-booking-toolkit' ); ?>
    										<# } else { #>
    											<?php esc_attr_e( 'Add Image', 'travel-booking-toolkit' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="travel-booking-toolkit-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'travel-booking-toolkit' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php esc_attr_e( 'No File Selected', 'travel-booking-toolkit' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'travel-booking-toolkit' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'travel-booking-toolkit' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_attr_e( 'Change File', 'travel-booking-toolkit' ); ?>
    										<# } else { #>
    											<?php esc_attr_e( 'Add File', 'travel-booking-toolkit' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
    					...
    					<# }); #>
    					<button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( 'Remove', 'travel-booking-toolkit' ); ?></button>
    				</div>
    			</li>
    		</script>
    		<?php
