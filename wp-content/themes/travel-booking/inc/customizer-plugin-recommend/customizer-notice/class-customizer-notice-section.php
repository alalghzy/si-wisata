<?php
/**
 * Travel_Booking Customizer Notification Section Class.
 *
 * @package Travel_Booking
 */

/**
 * Travel_Booking_Customizer_Notice_Section class
 */
class Travel_Booking_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		...
		foreach ( $travel_booking_recommended_plugins as $slug => $plugin_opt ) {
			...
			$travel_booking_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$travel_booking_recommended_plugin['class'] = 'active';
			} else {
				$travel_booking_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$travel_booking_recommended_plugin['button_class'] = 'install-now button';
					$travel_booking_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$travel_booking_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$travel_booking_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$travel_booking_recommended_plugin['button_class'] = 'deactivate-now button';
					$travel_booking_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$travel_booking_recommended_plugin['id'] = $slug;
			$travel_booking_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$travel_booking_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$travel_booking_recommended_plugin['description'] = $info->short_description;
			}

			$travel_booking_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $travel_booking_recommended_plugin;
