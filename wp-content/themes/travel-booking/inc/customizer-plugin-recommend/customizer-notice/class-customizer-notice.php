<?php
/**
 * Travel Agency Customizer Notification System.
 *
 ...
 */
class Travel_Booking_Customizer_Notice {
	...
	/**
	 * The single instance of Travel_Booking_Customizer_Notice
	 *
	 * @var Travel_Booking_Customizer_Notice $instance The Travel_Booking_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Travel_Booking_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Travel_Booking_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Travel_Booking_Customizer_Notice ) ) {
			self::$instance = new Travel_Booking_Customizer_Notice;
	...
	public function setup_config() {
		...
		global $travel_booking_recommended_plugins;
		...
		$travel_booking_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$travel_booking_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'travel-booking-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/css/customizer-notice.css', array(), TRAVEL_BOOKING_THEME_VERSION );
		...
		wp_enqueue_script( 'travel-booking-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/js/customizer-notice.js', array( 'customize-controls' ), TRAVEL_BOOKING_THEME_VERSION );
		wp_localize_script(
			'travel-booking-customizer-notice', 'customizer_notice_data', array(
				'ajaxurl'                  => admin_url( 'admin-ajax.php' ),
				'template_directory'       => get_template_directory_uri(),
				'base_path'                => admin_url(),
				'activating_string'        => __( 'Activating', 'travel-booking' ),
			)
		);
	...
	public function customize_register_notice( $wp_customize ) {
		...
		/**
		 * Include the Travel_Booking_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Travel_Booking_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Travel_Booking_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			$show_recommended_plugins = get_option( 'travel_booking_show_recommended_plugins' );
			...
			update_option( 'travel_booking_show_recommended_plugins', $show_recommended_plugins );
		}
