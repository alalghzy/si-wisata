...
class Travel_Booking_Plugin_Install_Helper {
		...
		if ( ! empty( $slug ) ) {
			...
			switch ( $state ) {
				case 'install':
					...
					$button .= '<a data-slug="' . esc_attr( $slug ) . '" class="install-now ta-install-plugin button  " href="' . esc_url( $nonce ) . '" data-name="' . $slug . '" aria-label="Install ' . $slug . '">' . __( 'Install and activate', 'travel-booking' ) . '</a>';
					break;
				...
				case 'activate':					
					...
					$button .= '<a data-slug="' . esc_attr( $slug ) . '" class="activate button button-primary" href="' . esc_url( $nonce ) . '" aria-label="Activate ' . $slug . '">' . __( 'Activate', 'travel-booking' ) . '</a>';
					break;
			}// End switch().
			$button .= '</div>';
		}// End if().
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'travel-booking-plugin-install-helper', get_template_directory_uri() . '/inc/customizer-plugin-recommend/plugin-install/js/plugin-install.js', array( 'jquery' ), TRAVEL_BOOKING_THEME_VERSION, true );
		wp_localize_script(
			'travel-booking-plugin-install-helper', 'plugin_helper',
			array(
				'activating' => esc_html__( 'Activating ', 'travel-booking' ),
			)
		);
		wp_localize_script(
